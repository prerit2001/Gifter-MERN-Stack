...
import { Item } from "./ImageGalery.element";
...
function ImageGallaryComponent() {
  ...
  useEffect(() => {
    ...
    // console.log(id._id);
    axios
      .post("https://node-backend-gifter.herokuapp.com/api/myallpost", data)
      .then((result) => {
        // console.log(result);
        setwish(result.data);
      });
  ...
  function deletewish() {
    toast.info("Please Wait Till Server Wakes", {
    ...
    axios
      .post("https://node-backend-gifter.herokuapp.com/api/deletewish", data)
      .then((result) => {
        //  console.log(result);
      ...
      .catch((error) => {
        toast.error("Sorry, Server not reachable!!", {
          position: toast.POSITION.TOP_CENTER,
        });
      });
  }
          ...
          return (
            <>
              ...
              >
                <Item
                  src={item.Pic}
                  style={{ width: "80%", height: "500px", maxWidth: "700px" }}
                />
                <br />
                <div
                  style={{
