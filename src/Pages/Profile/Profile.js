import React,{useEffect,useState} from 'react';
import OverlayTrigger from 'react-bootstrap/OverlayTrigger'
import Tooltip from 'react-bootstrap/Tooltip'
import {AiFillWechat} from 'react-icons/ai';
import {RiUserFollowFill} from 'react-icons/ri';
import {PostWish} from '../CreateWish/PostWish';
import ProfileNavbar from '../../Components/ProfileNavbar/Navbar'
import Axios from 'axios';
import Modal from 'react-modal';
import {FaWindowClose} from 'react-icons/fa';

import {
    Full,
    SemiFull,
    FullPage,
    ProfileImg,
    NameDisplay,
    FollowBox,
    Following,
    Followers,
    Wishes,
    NameDisplay1,
    Moto,
    PicChange,
    ChatLeft,
    MainScreen,
    Something,
    ChatRight,
    Heading1,
    Input,
    Button1,
    Button3,
    Img,
    Account
} from './Profile.element';

import './Profile.css';


import ImageGalery from '../../Pages/WishGalery/ImageGalery'
import { toast } from 'react-toastify';


const customStyles = {
  content : {
    top                   : '50%',
    left                  : '50%',
    right                 : 'auto',
    bottom                : 'auto',
    marginRight           : '-50%',
    transform             : 'translate(-50%, -50%)'
  }
};

export const Profile = () => {
  const USER = JSON.parse(localStorage.getItem('UserData'));
    const renderTooltip = (props) => (
        <Tooltip id="button-tooltip" {...props}>
          <PicChange>😍 Click to Change Profile Picture 😍</PicChange>
        </Tooltip>  
      );

      const [button,setButton] = useState(true);
      const [wishh,setwishh] = useState('');
      const [followers,setfollowers] = useState('');
      const [followings,setfollowings] = useState('');
      const [datailFollowers,setdetailFollowes] = useState([]);
      const [modalIsOpen,setIsOpen] = React.useState(false);
      const [updateName,setUpdateName] = useState('');
      const [updateBio,setUpdateBio] = useState('');
      const [updatePic,setUpdatePic] = useState('');
      const [Loading,setLoading] = useState(false);
      var url = '';

 
      const [allmessage,setallmessage] = useState([]);
      var array = [];

      const showButton = () => {
        if(window.innerWidth<960){
          setButton(false);
        }
        else{
          setButton(true);
        }
      }

      function openModal() {
        setIsOpen(true);
        
      }
     
      function afterOpenModal() {
        // references are now sync'd and can be accessed.
        subtitle.style.color = '#f00';
      }
     
      function closeModal(){
        setIsOpen(false);
      }
      
      useEffect(() => {
        const data = {
          _id : USER._id
        }
        Axios.post('https://node-backend-gifter.herokuapp.com/api/myallpost',data)
            .then(result=>{
              console.log(result);
              setwishh(result.data.length);
        })
        Axios.post('https://node-backend-gifter.herokuapp.com/api/detailUserFollower',data)
            .then(result=>{
              console.log(result);
              setdetailFollowes(result.data);
        })
        Axios.post('https://node-backend-gifter.herokuapp.com/api/userfollower',{id : USER._id})
        .then(result=>{
          console.log(result);
          setfollowers(result.data);
          console.log(result.data);
        }).catch(error=>{
          console.log(error);
        })
    
        Axios.post('https://node-backend-gifter.herokuapp.com/api/userfollowing',{id : USER._id})
        .then(result=>{
          console.log(result);
          setfollowings(result.data);
          console.log(result.data);
        }).catch(error=>{
          console.log(error);
        })

        Axios.post('https://node-backend-gifter.herokuapp.com/api/findallmessages',{id : USER._id})
        .then(result=>{
          console.log(result.data);
          setallmessage(result.data);
          
        }).catch(error=>{
          console.log(error);
        })
        showButton();
      }, [])
  
      window.addEventListener('resize',showButton);
      const [message,setmessage] = useState('');

      function SendMessage(){

        if(message.length<1 || message.length>=30){
          toast.error('Message Coloumn Error : Should be greater than 0 and less than 30 Characters',{position: toast.POSITION.TOP_CENTER});
          return;
        }

        window.location.hash = true;

         const data = {
           message: message,
           To : USER._id,
           From :  USER._id,
         }

         Axios.post('https://node-backend-gifter.herokuapp.com/api/sendmessage',data)
         .then(result=>{
           console.log(result);
           
           window.location.reload(false);
        
         })
          .catch(error=>{
            console.log(error);
          })
      }

      function updateProfile(){
        setLoading(true);

        if(updateName.length===0 || updateName.length>50){
          toast.error('Name Coloumn Error : Should be non-empty less than 50 characters',{position: toast.POSITION.TOP_CENTER});
          //return;
         setLoading(false); return;
        }
        if(updateBio.length===0 || updateBio.length>30){
          toast.error('Bio Coloumn Error : Should be non-empty less than 50 characters',{position: toast.POSITION.TOP_CENTER});
         // return;
          setLoading(false); return;
         }

        const  fileType = updatePic['type'];
        const validImageTypes = [ 'image/jpg','image/jpeg', 'image/png'];
        if (!validImageTypes.includes(fileType)) {
            toast.error('Invalid Image Type, Retry ?',{position: toast.POSITION.TOP_CENTER});
            setLoading(false);return;
        }

        var fileSize = updatePic['size'];
        if(fileSize>1000000){
            //  alert('Photo Size Exceeds , Size must be less than 500Kb');
            toast.error('Photo Size Exceeds , Size must be less than 1MB',{position: toast.POSITION.TOP_CENTER});
            setLoading(false); return;
        }
        setLoading(true);
        toast.info('Image Uploading ! Plese Wait !',{position: toast.POSITION.TOP_CENTER});
        const data = new FormData();
        data.append('file',updatePic);
        data.append('upload_preset','gifter');
        data.append('cloud_name','prerit-cloud');
        async function API(){
          const responce = await fetch('https://api.cloudinary.com/v1_1/prerit-cloud/image/upload',{
              method: 'post',
              body: data
          })
          .then(res=>res.json())
          .then(data=>{
              console.log(data);
              toast.success('Image Upload Sucessful ! Wait a Moment !',{position: toast.POSITION.TOP_CENTER});
              url = data.url;
              console.log(url)
              
          })
          .catch(err=>{
              console.log(err);
              toast.error('Image Upload Failed , Retry? ',{position: toast.POSITION.TOP_CENTER});
              setLoading(false);
              return;
          })
          
  
          function AXIOS(responce){
              const postwishdata = {
                  Name : updateName,
                  Moto : updateBio,
                  Pic: url,
                  Email: USER.Email
              }
              
              console.log(url)
              Axios.post('https://node-backend-gifter.herokuapp.com/api/updatedata',
              {
                Name : updateName,
                Moto : updateBio,
                url: url,
                Email: USER.Email
              })
              .then(function(responce){
                  console.log(responce);
                  toast.success('Added Sucessful',{position: toast.POSITION.TOP_CENTER});
                  localStorage.removeItem("UserData");
                  USER.Name = updateName;
                  USER.Moto = updateBio;
                  USER.pofilePicture = url;
                  localStorage.setItem("UserData",JSON.stringify(USER));
                    setLoading(false);
                  })
              .catch(function(error){
                  console.log(error);
                  toast.error(error,{position: toast.POSITION.TOP_CENTER});
                      setLoading(false);
                  }); 
              }
  
              AXIOS(responce);
      
          }
          
          API();
      }

    //console.log(array);
    var subtitle;
    
    return (
        <>  
            <ProfileNavbar />
            <Something>
            <ChatLeft>
              {button && <> 
                
                <h2 className="H22"> <AiFillWechat/> <br/> Recent Annonymous Chat Messages <br/> 💬 </h2>

                    <form class="form" >
                        <div class="animate__animated animate__fadeInLeft">
                            <input id="candidate" type="text" class="form__field" placeholder="Message Here(30 Words)" onChange={e=>setmessage(e.target.value)} pattern=".{1,30}" required/>
                            <button id="btn2" type="button" class="btn btn--primary btn--inside uppercase" onClick={SendMessage}>Send</button>
                        </div>
                    </form>
                    <br/>

                    {
                      allmessage.slice(0,50).map(item=>{
                        if(item.To!==item.From)
                        return (
                          
                          <div className="container">
                          <img src="data:image/jpeg;base64," alt="Avatar" style={{width: "100%"}}  />
                          <p style={{padding: "10px"}}>{item.message}</p>
                          </div>
                        )
                        else return(
                          <div className="container darker">
                          <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRW0Z9xI3iCiFQxLoqDJ-uhvLTc1gwTZEkQ-Q&usqp=CAU" alt="Avatar" className="right" style={{width: "100%"}}  />
                          <p  style={{padding: "10px"}}>{item.message}</p>
                          </div>
                        )
                      })
                    }
                   

                   

                    {/* <br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /><br /> */}
                </>
              }
            </ChatLeft>
            <ChatRight>
              {button && <> 
                
                <h2 className="H22"> <RiUserFollowFill/> <br/> User's Follower <br/> 😊💬😊 </h2>

                    
                    <br/>
                   
                    
                  
              {
                    
                    datailFollowers.map(item => {
                      return (
                        <a href={"/profile/"+item._id}>
                        <div className="container container2">
                          <img src="data:image/jpeg;base64," alt="Avatar" style={{width: "100%"}}  />
                          <p style={{marginTop: "10px"}}>{item.Name}</p>
                        </div>
                        </a>
                      )
                    })
              }

                

                    

                    

                  

                   </>
              }
            </ChatRight>
            <MainScreen>
            <FullPage>
                 <Full>
                    
                     
                     <h1 className="H23" style={{cursor: "pointer"}}><u>User Profile</u></h1>
                     <SemiFull>
                    <OverlayTrigger
                    placement="bottom"
                    delay={{ show: 0, hide: 400 }}
                    overlay={renderTooltip}
                >
                
                <ProfileImg src={USER.pofilePicture} onClick={openModal}/>
                </OverlayTrigger>
                        <NameDisplay1><b>👋 Welcome 👋</b></NameDisplay1><br/>
                        <NameDisplay>{USER.Name}</NameDisplay><br/>
                        <Moto>Bio : {USER.Moto}</Moto>
                        <Moto>Age : {USER.Age}</Moto>
                        <FollowBox>
                            <Followers>{followers} <br/> Followers</Followers>
                            <Wishes>{wishh} <br/> Wishes</Wishes>
                            <Following>{followings} <br/> Following</Following>
                           

                          
                        </FollowBox>
                    </SemiFull>
                    <hr/>
                    {
                      <>
                      

                      <PostWish />

                      </>
                    
                    }
                    
                    <hr/>
                   
                    
                <ImageGalery />
                   
                </Full>
            </FullPage>
            
               
            </MainScreen>
            </Something>

            <h1 className="H23" style={{cursor: "pointer",color: "white"}}>.</h1>



                    
      <Modal
          isOpen={modalIsOpen}
          onAfterOpen={afterOpenModal}
          onRequestClose={closeModal}
          style={customStyles}
          contentLabel="Example Modal"
        >
       
         <h2 ref={_subtitle => (subtitle = _subtitle)} > </h2>
         <div style={{right:0,top:0,margin: "30px",position:"fixed"}}><FaWindowClose onClick={closeModal} /></div>
         
           <Heading1>Update </Heading1>
          
         <Input type="text" placeholder="👨 Update Name"  onChange={e=>setUpdateName(e.target.value)}  pattern=".{1,30}" required/><br/>
         <Input type="text" placeholder="🤔 Update Bio" onChange={e=>setUpdateBio(e.target.value)} pattern=".{1,50}" required/>
                    <label style={{font: "20px" ,paddingLeft:"15px", marginBottom: "0px" ,color: "black"}} for="ii">Upload Profile Pic 👇</label>
         <Input  type="file" id="ii" placeholder="🤔 Updata Bio" pattern=".{1,50}" onChange={e=>setUpdatePic(e.target.files[0])} required/>
         
        
         {!Loading && <><Button1 onClick={updateProfile}>Update</Button1>
         <Account onClick={closeModal}>❌ cancel ❌</Account>
         </>}
         {Loading  && <><Button3><Img src="https://media.giphy.com/media/3o7bu3XilJ5BOiSGic/giphy.gif" style={{height: '60px',textAlign: 'center'}}/></Button3></>}
         
        
         


         

        </Modal>

        </>
    )
}

