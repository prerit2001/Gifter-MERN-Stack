...
function InfoSection({
  ...
  function Register(e) {
    ...
    axios
      .post("http://localhost:3001/api/signup", signpupdata)
      .then(function (responce) {
        // console.log(responce);
        toast.success("User Registered Sucessful", {
      ...
      .catch(function (error) {
        // console.log(error);
        toast.error(
          "Server Refused To Connect, Make Sure Email Not Registered Yet",
          { position: toast.POSITION.TOP_CENTER }
        );
        setloading(false);
      });
  ...
  function Login(e) {
    ...
    axios
      .post(
        "https://node-backend-gifter.herokuapp.com/api/signin",
        loginformdata
      )
      .then(function (responce) {
        localStorage.setItem("UserData", JSON.stringify(responce.data.user));
            ...
            <>
              ...
              <form onSubmit={Login}>
                <Input
                  type="email"
                  placeholder="📓 Email"
                  onChange={(event) => IsetEmail(event.target.value)}
                ...
                <Input
                  ...
                  onChange={(event) => IsetPassword(event.target.value)}
                  required
                />

                <ForgetPass onClick={handlePreview2}>
            ...
            <>
              ...
              <form onSubmit={Register}>
                <FullName
                  ...
                  onChange={(event) => UsetName(event.target.value)}
                  required
                ></FullName>
                <FullName
                  type="text"
                  placeholder="🤔 Bio"
                  onChange={(event) => UsetMoto(event.target.value)}
                  required
                ></FullName>
                <br />
                <Age
                  ...
                  name="phone"
                  pattern="[0-9]{10}"
                  onChange={(event) => UsetPhone(event.target.value)}
                  required
                ></Age>
                <Age
                  type="text"
                  placeholder="👨 Age"
                  pattern="[0-9]{2}"
                  onChange={(event) => UsetAge(event.target.value)}
                  required
                ></Age>
                <br />
                <Input1
                  ...
                  placeholder="📓 Email"
                  name="email"
                  onChange={(event) => UsetEmail(event.target.value)}
                  pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2, 4}$"
                  required
                />
                <Input1
                  ...
                  placeholder="🔑 Password"
                  name="password"
                  onChange={(event) => UsetPassword(event.target.value)}
                  required
                />
                <br />
                {!loading && (
          ...
          ForgetPassword === 1 && (
            <>
              ...
              <form>
                ...
                />
                <br />
                <Button1 type="submit" onClick={OTP}>
                  Request OTP
                </Button1>
